package com.web.commons.web;

import com.fasterxml.jackson.annotation.JsonIgnore;
import com.web.commons.config.Global;
import com.web.commons.utils.CookieUtils;
...
public class Page<T> {
	...
	/***
	 * Add By Nemo on 2016/06/28 分页按钮显示的个数调整
	 * @param mlength
     */
	public void setBtnLength(int mlength){
		this.length = mlength;
	}
	...
	public Page(HttpServletRequest request, HttpServletResponse response,
		...
		String defSize = request.getParameter("defSize");//768 默认行数
		String rowHeight = request.getParameter("rowHeight");//行高度 默认25
		String size = request.getParameter("pageSize");
		String ratioHeight = request.getParameter("ratioHeight");//分辨率高度
		if(StringUtils.isNumeric(ratioHeight)&&StringUtils.isNumeric(defSize)&&!ratioHeight.trim().equals("768")){
			//如果需要计算，计算后就把pageSize替换掉
			Integer rheight=StringUtils.isNumeric(rowHeight)?Integer.valueOf(rowHeight):25;
			Integer psize=((Integer.valueOf(ratioHeight)-768)/rheight)+Integer.valueOf(defSize);
			size=psize.toString();//计算后赋值给PageSize
		}else if(StringUtils.isNumeric(defSize)){
			size=defSize;//计算后赋值给PageSize
		}
	...
	public String toString() {
		...
		sb.append(funcName + "(this.value," + pageSize
				+ ");\" onclick=\"this.select();\"/> / <input type=\"text\" value=\"" + ((int)Math.ceil(count / (double)pageSize))
				+ "\" class=\"uneditable-input\" /> 页&nbsp;&nbsp;每页 ");
		sb.append("<input type=\"text\" value=\""
				+ pageSize
				+ "\" onkeypress=\"var e=window.event||this;var c=e.keyCode||e.which;if(c==13)");
...
//	}
	...
	public String leftPage(){
		...
		StringBuilder sb = new StringBuilder();
		if (pageNo == first) {// 如果是首页
			sb.append("<a href=\"javascript:vord(0)\" disabled =\"true\">&#171; 上一页</a>\n");
		} else {
		...
		sb.append("<select onchange=\"$(this[selectedIndex]).click()\">");
		for (int i = begin; i <= end; i++) {
			...
			if (i == pageNo) {
				...
				sb.append("<option onclick=\"" + funcName
						+ "(" + i + "," + pageSize + ");\">" + (i + 1 - first)
						+ "</option>\n");
			}
		}
		sb.append("</select>");
		if (pageNo == last) {
			sb.append("<a href=\"javascript:vord(0)\" disabled =\"true\">下一页 &#187;</a>\n");
		} else {
			sb.append("<a href=\"javascript:\" onclick=\"" + funcName + "("
					+ next + "," + pageSize + ");\">" + "下一页 &#187;</a>\n");
		}
		sb.insert(0, "<nav><ul class=\"pagination\">\n").append("</ul></nav>\n");
		sb.append("<div style=\"clear:both;\"></div>");
