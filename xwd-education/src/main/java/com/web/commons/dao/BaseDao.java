...
public class BaseDao<T> {
	...
	public List<T> findAllList() {
		return find("from " + entityClass.getSimpleName() + " where delFlag='"
				+ BaseEntity.DEL_FLAG_NORMAL + "' order by sort");
	}

	public Object finfByHQL(String qlString, Parameter parameter) {
		Query query = createQuery(qlString, parameter);
	 ...
	 */
	/*
	public List<T> findWorksite() {
		return find(" OBJECTID,LINESECTION,REMARKS,MEASURE,车站 station,LINE,CODE_NEW,NAME, sde.st_minx (SHAPE)  minx,sde.st_maxx (SHAPE) maxx, sde.st_miny (SHAPE) miny,sde.st_maxy (SHAPE) maxy from sde.worksite ");
	}
	*/
	/**
	 * 获取实体
