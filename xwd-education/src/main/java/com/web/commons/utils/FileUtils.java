 ...
 * 换成apache-common-compress包以后好多内容需要重写！！！所以先标示成废弃
 * 
 * @deprecated
 * 
 * @author ThinkGem\nanxiaoqiang
...
public class FileUtils extends org.apache.commons.io.FileUtils {
	...
	// try {
	// File zipFile = new File(descFileName);
	// zaos = new ZipArchiveOutputStream(zipFile);
	// // Use Zip64 extensions for all entries where they are required
	// zaos.setUseZip64(Zip64Mode.AsNeeded);
	// if ("*".equals(fileName) || "".equals(fileName)) {
	// // 多个文件
	// FileUtils.zipDirectoryToZipFile(dirPath, fileDir, zaos);
	// } else {
	// File file = new File(fileDir, fileName);
	// if (file.isFile()) {
	// FileUtils.zipFilesToZipFile(dirPath, file, zaos);
	// } else {
	// FileUtils.zipDirectoryToZipFile(dirPath, file, zaos);
	// }
	// }
	// zaos.finish();
	// } catch (Exception e) {
