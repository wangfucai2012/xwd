...
<head>
    ...
    <script>
        ...
        function page(n, s) {
            $("#pageNo").val(n);
            $("#pageSize").val(s);
            $("#searchForm").attr("action", "${ctx}/device/electricity/cne/jqline?catalog=${catalog}");
            $("#searchForm").submit();
            return false;
        }
    </script>
...
<ul class="nav nav-tabs">
    <li class="active"><a href="${ctx}/device/electricity/cne/jqline?catalog=${catalog}">JQ线列表</a></li>
    <li><a href="${ctx}/device/electricity/cne/jqline/form?catalog=${catalog}">JQ线添加</a></li>
</ul>

<!--查询框start-->
<form:form id="searchForm" modelAttribute="jqLine"
           method="post"
...
<table id="contentTable" class="table table-striped table-bordered table-hover">
    ...
    <c:forEach items="${page.list}" var="list" varStatus="i">
        <tr>
            ...
            <shiro:hasPermission name="sys:user:edit">
                <td style="text-align: center;"><a href="${ctx}/device/electricity/cne/jqline/form?id=${list.id}&catalog=${catalog}">修改</a> <a
                        href="${ctx}/device/electricity/cne/jqline/delete?id=${list.id}&catalog=${catalog}"
