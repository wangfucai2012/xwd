...
<script>
	...
	function page(n, s) {
	    $("#pageNo").val(n);
	    $("#pageSize").val(s);
	    //$("#searchForm").attr("action", "${ctx}/data/comm/cameralocation");
	    $("#searchForm").submit();
	    return false;
	}
</script>
...
<body>
	...
	<!--查询框start-->
	<form:form id="searchForm" modelAttribute="cameraLocation"
	           action="${ctx}/data/comm/cameralocation?catalog=${entity.cat.id}" method="post"
	    ...
	    <input id="cat" type="hidden" name="cat.id" value="${cameraLocation.cat.id}"/>
		<label>名称 ：</label>
