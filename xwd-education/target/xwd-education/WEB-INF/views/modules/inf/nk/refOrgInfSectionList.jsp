...
<body>
	<div class="searchBar">	
		<form:form id="searchForm" modelAttribute="refOrgInfSection"
			action="${ctx}/inf/nk/reforginfsection/" method="post"
			...
			<div>
				<label>部门ID：</label>
				<form:input path="idDepartment" htmlEscape="false" maxlength="50" class="input-small"/>
	 ...
	 <thead>	
		<tr>
			...
			<th style="text-align: center;">区间id</th>
		...
		<c:forEach items="${page.list}" var="entity">
			...
			<tr>
                ...
                <c:set var="string3" value="${fn:substring(string1, 3,6)}"/>K${string2}+${string3}
                 ...
                 --%> 
				<td style="text-align: center;">${entity.idDepartment}</td>
				<td style="text-align: center;">${entity.idBlock}</td>
				<td style="text-align: center;">${entity.createTime}</td>
				<td style="text-align: center;">${entity.modifyTime}</td>
				
				<td style="text-align: center;">
				<a href="${ctx}/inf/nk/reforginfsection/view?id=${entity.id}">查看</a>
                </td>
			</tr>
		</c:forEach>
		</tbody>
