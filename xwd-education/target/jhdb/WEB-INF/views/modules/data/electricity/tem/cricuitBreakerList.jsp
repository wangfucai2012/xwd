...
<head>
    ...
    <script>
        ...
        function page(n, s) {
            $("#pageNo").val(n);
            $("#pageSize").val(s);
            $("#searchForm").attr("action", "${ctx}/device/electricity/tem/cricuitbreaker?catalog=${catalog}");
            $("#searchForm").submit();
            return false;
        }
    </script>
...
<ul class="nav nav-tabs">
    <li class="active"><a href="${ctx}/device/electricity/tem/cricuitbreaker?catalog=${catalog}">断路器列表</a></li>
    <li><a href="${ctx}/device/electricity/tem/cricuitbreaker/form?catalog=${catalog}">断路器添加</a></li>
</ul>

<!--查询框start-->
<form:form id="searchForm" modelAttribute="cricuitBreaker"
           method="post"
...
<table id="contentTable" class="table table-striped table-bordered table-hover">
    ...
    <c:forEach items="${page.list}" var="list" varStatus="i">
        <tr>
            ...
            <shiro:hasPermission name="sys:user:edit">
                <td style="text-align: center;"><a href="${ctx}/device/electricity/tem/cricuitbreaker/form?id=${list.id}&catalog=${catalog}">修改</a> <a
                        href="${ctx}/device/electricity/tem/cricuitbreaker/delete?id=${list.id}&catalog=${catalog}"
