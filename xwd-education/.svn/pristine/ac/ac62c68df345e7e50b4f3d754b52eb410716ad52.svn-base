...
<head>
    ...
    <script>
        ...
        function page(n, s) {
            $("#pageNo").val(n);
            $("#pageSize").val(s);
            $("#searchForm").attr("action", "${ctx}/device/electricity/cne/guardLine");
            $("#searchForm").submit();
            return false;
        }
    </script>
...
<ul class="nav nav-tabs">
    <li class="active"><a href="${ctx}/device/electricity/cne/guardLine?catalog=${catalog}">保护线列表</a></li>
    <li><a href="${ctx}/device/electricity/cne/guardLine/form?catalog=${catalog}">保护线添加</a></li>
</ul>

<!--查询框start-->
<form:form id="searchForm" modelAttribute="guardLine"
           method="post"
...
<table id="contentTable" class="table table-striped table-bordered table-hover">
    ...
    <c:forEach items="${page.list}" var="list" varStatus="i">
        <tr>
            ...
            <shiro:hasPermission name="sys:user:edit">
                <td><a href="${ctx}/device/electricity/cne/guardLine/form?id=${list.id}&catalog=${catalog}">修改</a> <a
                        href="${ctx}/device/electricity/cne/guardLine/delete?id=${list.id}&catalog=${catalog}"
