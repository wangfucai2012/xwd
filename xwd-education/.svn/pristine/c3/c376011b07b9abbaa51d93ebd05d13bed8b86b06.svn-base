...
<body>
	<div class="searchBar">	
		<form:form id="searchForm" modelAttribute="contactNetPro"
			action="${ctx}/inf/nk/contactnetpro/" method="post"
			...
			<div>
				<label>问题项：</label>
				<form:input path="problemName" htmlEscape="false" maxlength="50" class="input-small"/>
	 ...
	 <thead>	
		<tr>
			...
			<th style="text-align: center;">问题项</th>
			<th style="text-align: center;">问题类型</th>
			<th style="text-align: center;">区间名称</th>
		...
		<c:forEach items="${page.list}" var="entity">
			...
			<tr>
                ...
                <c:set var="string3" value="${fn:substring(string1, 3,6)}"/>K${string2}+${string3}
                 ...
                 --%> 
				<td style="text-align: center;">${entity.problemName}</td>
				<td style="text-align: center;">${entity.problemType}</td>
				<td style="text-align: center;">${entity.blockName}</td>
				<td style="text-align: center;">${entity.departmentName}</td>
				
				<td style="text-align: center;">
				<a href="${ctx}/inf/nk/contactnetpro/view?id=${entity.id}">查看</a>
                </td>
			</tr>
		</c:forEach>
		</tbody>
