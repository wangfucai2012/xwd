...
<body>
	...
	<form:form id="searchForm" action="${ctx}/sys/log/" method="post" class=" form-search">
		...
		<div>
			<label>用&nbsp;&nbsp;户&nbsp;&nbsp;ID：&nbsp;</label><input id="createById" name="createById" type="text" maxlength="50" class="input-medium" value="${createById}"/>
			...
			&nbsp;&nbsp;&nbsp;<input id="btnSubmit" class="btnSubmit" type="submit" value="查询"/>&nbsp;&nbsp;<label for="exception"><input id="exception" name="exception" type="checkbox"${exception eq '1'?' checked':''} value="1"/>只查询异常信息</label>
		</div>
	</form:form>
	</div>
